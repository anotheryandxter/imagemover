#!/usr/bin/env python3
"""
File Watchdog Auto Organizer - v1.6 RECURSIVE SUBDIRECTORY VERSION
Monitors source folders AND ALL SUBFOLDERS and moves files to timestamped session folders.

Architecture (v1.6):
- RECURSIVE: Scans subdirectories within source folders
- CLEANUP: Automatically removes empty subfolders after moving
- CLEAN: Same simple architecture as v1.5
- RELIABLE: Proven logic with enhanced directory handling

Main Loop:
1. Detect files in ALL source folders (including subfolders)
...
CONFIG = {
    ...
    # NOTE: Script will scan subfolders within these sources
    "SOURCE_FOLDERS": [
    ...
    "RECURSIVE_SCAN": True,  # NEW: Scan subdirectories
    "AUTO_CLEANUP_EMPTY_DIRS": True,  # NEW: Remove empty folders
...
def get_eligible_files(folder_path: Path, recursive: bool = True) -> List[Path]:
    """
    Get files ready to move from a folder (and subfolders if recursive)
    
    NEW: Supports recursive subdirectory scanning
    """
    eligible = []
    
    try:
        if recursive:
            # Scan recursively (including subfolders)
...
def scan_all_sources() -> Dict[str, List[Path]]:
    """Scan all source folders (and subfolders), return files per folder"""
    result = {}
    ...
    for source_folder in CONFIG["SOURCE_FOLDERS"]:
        ...
        files = get_eligible_files(source_path, recursive=CONFIG.get("RECURSIVE_SCAN", True))
...
# ============================================================================
# CLEANUP FUNCTIONALITY (NEW)
# ============================================================================

def cleanup_empty_directories(source_folder_path: Path):
    """
    Recursively remove empty subdirectories within source folder
    
    NEW: Cleans up after files are moved
    Leaves empty subdirectories hierarchy for user to clean manually if needed,
    but removes truly empty directories
    """
    try:
        # Walk through all directories (bottom-up so we delete empty dirs first)
        for dirpath, dirnames, filenames in os.walk(source_folder_path, topdown=False):
            ...
            # Only process if directory is truly empty (no files, no subdirs)
            try:
                if not os.listdir(dirpath):  # Directory is empty
                    os.rmdir(dirpath)
                    logger.info(f"🗑️  Removed empty directory: {Path(dirpath).relative_to(source_folder_path)}")
            except OSError:
                # Directory is not empty or permission denied, skip it
                pass
    
    except Exception as e:
        logger.error(f"Error cleaning up directories in {source_folder_path}: {e}")

def cleanup_all_sources():
    """Cleanup empty directories in all source folders"""
    if not CONFIG.get("AUTO_CLEANUP_EMPTY_DIRS", True):
        return
...
def main_loop():
    ...
    Steps:
    1. Detect files (including subfolders)
    ...
    while True:
        try:
            # ===== STEP 1: DETECT FILES (RECURSIVE) =====
            logger.info("🔍 Scanning source folders (including subfolders)...")
...
def print_header():
    ...
    print("\n" + "=" * 70)
    print("📁 File Watchdog Auto Organizer (v1.6 - RECURSIVE SUBDIRECTORIES)")
    print("=" * 70)
    print(f"📍 Monitoring {len(CONFIG['SOURCE_FOLDERS'])} source folders:")
    for folder in CONFIG['SOURCE_FOLDERS']:
        print(f"   • {Path(folder).name}")
        if CONFIG.get("RECURSIVE_SCAN", True):
            print(f"     └─ (Including all subfolders)")
    ...
    if CONFIG.get("AUTO_CLEANUP_EMPTY_DIRS", True):
        print(f"🗑️  Auto cleanup: Empty subdirectories (enabled)")
